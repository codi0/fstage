...
var config = {
	...
	name:  'My App',   // app name used in notification titles
...
function buildSensitiveParams(input) {
	var defaults = ['token','auth','signature','expires','x-amz-signature','x-amz-credential','x-amz-security-token','googleaccessid'];
...
function trimCache(cache, maxEntries) {
	...
	return cache.keys().then(function(keys) {
		...
		return Promise.all(keys.slice(0, keys.length - maxEntries).map(function(req) { return cache.delete(req); }));
	});
}
...
function makeOfflineResponse(status, isApi) {
	status = status || 503;
	var body = isApi ? JSON.stringify({ error: 'offline', status: status, offline: true }) : 'Offline';
	return new Response(body, {
		status: status, statusText: 'Offline',
...
function fetchWithPolicy(request, policy) {
	...
	if (policy && policy.mode === 'same-origin') options.credentials = 'same-origin';
	else if (policy && policy.canCache)          options.credentials = 'omit';
...
self.addEventListener('message', function(e) {
	...
	if (e.data === 'clearCaches') {
		e.waitUntil(
			caches.keys()
				.then(function(keys) {
					return Promise.all(keys.filter(isManagedCache).map(function(key) { return caches.delete(key); }));
				})
				.then(function() {
...
self.addEventListener('fetch', function(e) {
	...
	if (shouldBypassCache(request, url)) {
		e.respondWith(fetch(request).catch(function() { return makeOfflineResponse(isNav ? 503 : 504, false); }));
		return;
	}
	...
	if (!policy.canCache || !policy.cacheName) {
		e.respondWith(fetch(request).catch(function() { return makeOfflineResponse(504, false); }));
		return;
	}
	...
	e.respondWith(
		caches.open(policy.cacheName).then(function(cache) {
			return cache.match(request, { ignoreSearch: ignoreSearch }).then(function(cached) {
				var revalidate = fetchAndCache(cache, policy.cacheName, request, policy).catch(function() { return null; });
				if (cached) { e.waitUntil(revalidate); return cached; }
				return revalidate.then(function(response) { return response || makeOfflineResponse(504, false); });
			});
		})
	);
});
		...
		clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function(clientList) {
			...
			if (payload.notification && canSend) {
				return self.registration.showNotification(payload.notification.title, {
					body: payload.notification.body || '', tag: payload.notification.tag || '',
					data: payload.data || {}, badge: config.icon, vibrate: config.vibrate,
				});
			}
			return true;
		})
