...
This policy is mandatory for all Fstage modules (e.g. @fstage/*). Apps built on top of Fstage SHOULD follow it, but may deviate — for example, app code may use `const`/`let`, arrow functions, and `for...of` where it improves readability, provided the runtime minimum in Section 2 is still met.
